...
| Skill | 说明 |
|---|---|
| [chatgpt-remote-mcp](chatgpt-remote-mcp/SKILL.md) | 把本地机器通过远程 MCP 接到 ChatGPT/Claude 网页端，用网页端额度驱动本地干活。含 ChatGPT 对 URL 的隐藏校验规则（裸 IP 和 sslip.io 会被拒，报错却是误导性的 "does not implement OAuth"）、中国大陆 ICP 备案拦截的实测边界、Let's Encrypt 在境内服务器上的签发路径、DevSpace 配置、macOS 两层权限（allowedRoots + TCC）、反向隧道与 launchd 保活 |
